var customerList = (function ($, loggingDisplay) {
    'use strict';
    ...
    function loadFilterValueDropdown(filterByValue, filterValue, ignoreTrace) {
        ...
        if (filterByValue !== 'NoFilter') {
            ...
            $.ajax({
                url: filterApiUrl,
                data: { FilterBy: filterByValue, FilterValue: filterValue }
            })
    ...
    return {
        ...
        searchByCustomerName: function (filterElem ) {
            var filterByValue = $(filterElem).find(":text").val();
            loadFilterValueDropdown(filterByValue);
        },
